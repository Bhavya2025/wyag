...
# --- Chapter 10: The check-ignore command ----------------------------------
...
def gitignore_parse1(raw):
    # Parse one ignore line into (pattern, should_ignore) or None.
    raw = raw.strip()

    if not raw or raw[0] == "#":
        return None                 # blank line or comment
    elif raw[0] == "!":
        return (raw[1:], False)     # negation: explicitly DON'T ignore
    elif raw[0] == "\\":
        return (raw[1:], True)      # escaped leading '!' or '#'
    else:
        return (raw, True)
...
class GitIgnore(object):
    absolute = None    # rules that apply everywhere (global/exclude)
    scoped = None      # {directory: rules} from .gitignore files
...
def gitignore_read(repo):
    ...
    # Repo-local, never-committed rules: .git/info/exclude
    repo_exclude = os.path.join(repo.gitdir, "info/exclude")
    if os.path.exists(repo_exclude):
        with open(repo_exclude, "r") as f:
            ret.absolute.append(gitignore_parse(f.readlines()))

    # Global rules: $XDG_CONFIG_HOME/git/ignore (or ~/.config/...)
    ...
    # .gitignore files that are STAGED in the index, keyed by their dir.
    index = index_read(repo)
...
def check_ignore1(rules, path):
    # Apply one ruleset; the LAST matching rule wins (so later negations
    # can override earlier ignores). None means "no rule matched".
    result = None
...
def check_ignore_scoped(rules, path):
    # Walk up from the path's directory toward the root, applying the
    # nearest .gitignore that has an opinion.
...
def check_ignore_absolute(rules, path):
    for ruleset in rules:
        ...
        if result != None:
            return result
    return False  # nothing matched: not ignored
...
def check_ignore(rules, path):
    ...
    # Scoped (.gitignore) rules take precedence over the absolute ones.
